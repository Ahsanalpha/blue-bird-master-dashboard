...
import { BarChart3, Building2, Settings, HelpCircle, LogOut, Menu } from "lucide-react"
    ...
    <>
      ...
      >
        <div className="flex h-14 items-center border-b px-4">
          <span className="text-lg font-semibold">Tenants Manager</span>
              ...
              {footerItems.map((item, index) =>
                item.name === "Logout" ? (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium hover:bg-accent hover:text-accent-foreground text-muted-foreground w-full text-left",
                    )}
                  >
                    <item.icon className="h-5 w-5" />
                    <span>{item.name}</span>
...
const navItems = [
  { name: "Tenants Stats", href: "/dashboard", icon: BarChart3 },
  { name: "Tenants", href: "/tenants", icon: Building2, badge: "12" },
]

const footerItems = [
  { name: "Settings", href: "/settings", icon: Settings, description: "Admin settings" },
